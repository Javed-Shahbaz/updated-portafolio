...
const Skills = ({ setIsOpen, pdfPath }) => {
  const { stack, pAbout, softSkills, prospect } = useSelector(
    (state) => state.cards
  );
  const [isContentVisible, setIsContentVisible] = useState(false);
  const [isContentVisibleSkills, setIsContentVisibleSkills] = useState(false);
  const [isContentVisibleProspects, setIsContentVisibleProspects] =
    useState(false);
  ...
  return (
    <>
      <section id="about-skills">
        <h2 className="h2-card">Soft & Tech Stack Skills</h2>
        <div className="skills-container">
          <div className="skills-sidebar">
            <div className="resume">
              ...
              <hr
                className={
                  isContentVisible ? "toggle-resume" : " toggle-resume hidden"
                }
                onClick={toggleResume}
              />
              <div className="presentation-profile">
                {isContentVisible &&
                  pAbout.map((paragraph, index) => (
            ...
            <div className="prospect">
              ...
              <hr
                className={
                  isContentVisibleProspects
                    ? "toggle-prospect"
                    : " toggle-prospect hidden"
                }
                onClick={toggleProspects}
              />
              <div
                className={`presentation-profile ${
                  isContentVisibleProspects ? "" : "hidden"
                }`}
              >
                {isContentVisibleProspects && (
                  <p className="paragraph">{prospect}</p>
                )}
              </div>
            </div>
          </div>
